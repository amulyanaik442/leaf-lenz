"""Re-split wheat train data into 80/10/10 using ALL images (no undersampling)."""
import os
import sys
import random
import shutil
...
WHEAT_DATA_SRC = os.path.join(os.path.dirname(__file__), "wheat_data")
...
def main():
    print("=" * 60)
    print("  RE-SPLITTING WHEAT DATASET (80/10/10, FULL DATA)")
    print("=" * 60)

    raw_train_dir = os.path.join(WHEAT_DATA_SRC, "train")
    if not os.path.exists(raw_train_dir):
        print(f"ERROR: No raw train directory found at {raw_train_dir}")
        return
    ...
    # Create new splits - NO undersampling
    random.seed(SEED)
    print(f"\nCreating 80/10/10 splits (all data):")
    ...
    print("  Class weights will handle imbalance in training.")
