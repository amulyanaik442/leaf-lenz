"""
Build the Papaya training/validation/test working set from the audited images of
the BDPapayaLeaf (Mendeley) dataset (2,159 images, 5 classes).

Approved plan (audit 2026-08-05):
  * Raw dataset (original images only; annotations are separate, unused).
  * Exact-MD5 dedup: keep ONE copy per duplicate group; for the 224
    cross-class BacterialSpot<->Curl label conflicts keep the Curl copy and
    drop the conflicting BacterialSpot copy (approved).
  * No blur filter (only 10 images flagged, threshold heuristic not in the
    approved plan for this dataset).
...
Outputs:
  * dataset/papaya/Papaya_Split/{train,valid,test}/papaya___<class>/*.jpg
  * dataset/papaya/papaya_train_prep_report.json

The original BDPapayaLeaf archive is left untouched.
"""
import os
import sys
import json
...
SOURCE_DIR = os.path.join(BASE_DIR, 'dataset', 'papaya', 'BDPapayaLeaf', 'Original Images')
PAPAYA_DIR = os.path.join(BASE_DIR, 'dataset', 'papaya')
SPLIT_DIR = os.path.join(PAPAYA_DIR, 'Papaya_Split')
REPORT = os.path.join(PAPAYA_DIR, 'papaya_train_prep_report.json')
...
# For cross-class MD5 conflicts: which class's copy wins.
PREFERRED_CLASS = 'Curl'
...
def deduplicate(files):
    """
    Exact-MD5 dedup keeping one copy per group; for cross-class label
    conflicts keep the PREFERRED_CLASS copy.
    ...
    for h, g in groups.items():
        ...
        if len(classes) > 1:
            keep = sorted(g, key=lambda x: 0 if x[0] == PREFERRED_CLASS else 1)[0]
            final.append(keep)
            for item in g:
                if item is not keep:
...
def main():
    ...
    # ---- cleanup (MD5 dedup + label conflicts, prefer Curl) ----
