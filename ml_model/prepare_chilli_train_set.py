"""
Build the Chilli leaf training/validation/test working set from the audited
real (non-augmented) images of ravindubandara3002/chilli-plant-diseases-dataset.

Approved plan: 6 classes (drop Veinal Mottle [0 real] and Damping Off [36 real]),
REAL images only (no synthetic), balanced by undersample (seed 42).

Steps (seed-fixed for reproducibility):
  1. Enumerate REAL images from the extracted dataset (train+valid+test).
  2. Deduplicate exact MD5 copies:
       - same file in DIFFERENT classes -> label conflict -> drop all members
       - same file within one class  -> keep 1 representative
  3. Build dHash near-duplicate connected components (<= 8 bits) -> leakage units.
  4. Blur filter (Laplacian variance < 20, mirrors cacao).
...
Outputs:
  * dataset/chilli/Chilli_Split/{train,valid,test}/Chilli___<class>/*.jpg
  * dataset/chilli/chilli_train_prep_report.json

The extracted archive on Z: is left untouched.
"""
import os
import sys
import json
...
CHILLI_DIR = os.path.join(BASE_DIR, 'dataset', 'chilli')
SPLIT_DIR = os.path.join(CHILLI_DIR, 'Chilli_Split')
REPORT = os.path.join(CHILLI_DIR, 'chilli_train_prep_report.json')
...
def is_augmented_name(fname):
    return bool(AUG_PREFIX_RE.match(fname) or AUG_SUFFIX_RE.search(fname))


def load_real_images():
    """Return list of (cls, abspath, fname) for REAL images of the 6 classes."""
    files = []
    for split in ('train', 'valid', 'test'):
        ...
        for cls in CLASS_MAP:
            ...
            for f in sorted(os.listdir(d)):
                if f.lower().endswith(IMAGE_EXTS) and not is_augmented_name(f):
                    files.append((cls, os.path.join(d, f), f))
    return files


def load_downscaled_gray(path, max_dim=ANALYSIS_MAX_DIM):
    """Decode once to a downscaled grayscale numpy array.

    Uses cv2 reduced-read flags (fast for very large images) then resizes to
    a max dimension for a resolution-normalized, consistent analysis metric.
    """
...
def dedup(files):
    """files: list of (cls, path, fname). Returns (kept, dropped_report)."""
    ...
    for h, members in md5_map.items():
        ...
        if len(classes) > 1:
            ...
            keep_idx.add(min(members))  # keep 1 representative (lowest index)
    kept = [files[i] for i in sorted(keep_idx)]
    return kept, dropped


def build_components(kept):
    """Union images into leakage groups by dHash (<= threshold, same class)."""
    hashes = []
...
def assign_split(groups, rng):
    """Assign whole component groups to splits keeping per-class balance."""
...
def main():
    log('=' * 60)
    log('CHILLI TRAIN SET PREP (real-only, dedup -> blur -> balance -> split -> augment)')
    log('=' * 60)
    rng = rng_seeded(SEED)
    aug = make_augmenter()

    files = load_real_images()
    raw_counts = Counter(cls for cls, _, _ in files)
    log(f'  real images (6 classes): {len(files)}  {dict(raw_counts)}')
    ...
    report = {
        'classes': CLASS_MAP,
        'raw_real_counts': dict(raw_counts),
        'dedup_kept': len(kept),
