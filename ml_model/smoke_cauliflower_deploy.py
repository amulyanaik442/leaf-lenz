"""Post-deploy smoke test for the Cauliflower integration.

1. Cauliflower routing: each Cauliflower_Split/test image via TTA predict with
   prefix='cauliflower___' must return a cauliflower___ label.
2. Zero regression (crop-filtered): for a sample of existing-crop images, the
   crop-filtered top-1 from the NEW (141-class) model must exactly match the
   PRE-DEPLOY backup (138-class) model.
3. App wiring: predict_by_crop(..., 'cauliflower') routes to the general model
   and returns a cauliflower___ label; cauliflower crop key is registered.
"""
import os
import sys
import json
...
BACKUP_DIR = sorted(glob.glob(os.path.join(ML_ASSETS, 'backup_pre_cauliflower_*')))[-1]
...
def crop_prefix(crop):
    return {'cacao': 'Cacao___', 'tea': 'Tea___', 'pumpkin': 'Pumpkin___',
            ...
            'black_pepper': 'BlackPepper___', 'ginger': 'ginger___',
            'bean': 'bean___'}[crop]
...
def main():
    ...
    # ---- 1. cauliflower routing ----
    print('\n=== CAULIFLOWER ROUTING (Cauliflower_Split/test, prefix="cauliflower___") ===')
    test_root = os.path.join(BASE_DIR, 'dataset', 'cauliflower', 'Cauliflower_Split', 'test')
    total = correct = 0
    for cls in sorted(os.listdir(test_root)):
        ...
        ok = 0
        for p in imgs:
            label, conf = tta_predict(new_sess, new_names, p, prefix='cauliflower___')
            total += 1
            is_corr = (label == cls)
            ok += int(is_corr)
            correct += int(is_corr)
    ...
    print(f'  cauliflower test accuracy: {correct}/{total} = {100*correct/total:.1f}%')
    ...
    crops = {
        ...
        'ginger': os.path.join(BASE_DIR, 'dataset', 'ginger', 'Ginger_Split', 'test'),
        'bean': os.path.join(BASE_DIR, 'dataset', 'bean', 'Bean_Split', 'test'),
    }
    total_mismatch = 0
    samples = 0
    ...
    try:
        ...
        print(f'  cauliflower in CROP_CLASS_PREFIXES: {CROP_CLASS_PREFIXES.get("cauliflower")}')
        sample = None
        for cls in sorted(os.listdir(test_root)):
            d = os.path.join(test_root, cls)
            for f in sorted(os.listdir(d)):
        ...
        with open(sample, 'rb') as fh:
            label, conf, top5, mm = predict_by_crop(fh, 'cauliflower')
        print(f'  predict_by_crop(cauliflower): {label} ({conf:.3f})')
        assert label.startswith('cauliflower___'), 'expected a cauliflower___ label'
        with open(sample, 'rb') as fh:
            label, conf, top5, mm = predict_by_crop(fh, 'bean')
        print(f'  predict_by_crop(bean): {label} ({conf:.3f})')
        with open(sample, 'rb') as fh:
