"""
Train the Turmeric leaf classifier head with a strict zero-regression approach.

Approach (approved, mirrors chilli): freeze all 123 existing classifier rows,
train ONLY the 4 new Turmeric rows (indices 123-126) on the prepared Turmeric
crops, then splice the trained rows into a candidate ONNX. Existing rows remain
byte-identical.

Pipeline:
  1. Prepared splits: `dataset/turmeric/Turmeric_Split` (train/valid/test,
  ...
  4. Append the trained rows to a candidate ONNX (127 classes) + candidate
     class names.  Production model and class_names.json are NOT modified.

Verification:
  * zero_regression_check.json  - byte-identical non-Turmeric rows.
  * inference regression spot check on existing non-Turmeric crops (logits for
    the first 123 classes must be identical; report any Turmeric FPs).

Outputs (`ml_model/turmeric_retrain_output/`):
  * turmeric_candidate.onnx + candidate_class_names.json
...
Usage:
    python ml_model/train_turmeric.py
"""
import os
import sys
import json
...
SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'turmeric', 'Turmeric_Split')
FEAT_CACHE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'turmeric_features.pkl')
FEAT_CACHE_TTA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'turmeric_features_tta.pkl')
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'turmeric_retrain_output')
CKPT_DIR = os.path.join(OUTPUT_DIR, 'checkpoints')

# ---------------------------------------------------------------------------
# Config
# ---------------------------------------------------------------------------
TURMERIC_CLASS_NAMES = [
    'Turmeric___aphids_disease',
    'Turmeric___blotch',
    'Turmeric___healthy',
    'Turmeric___leaf_spot',
]
N_NEW = len(TURMERIC_CLASS_NAMES)
...
# ---------------------------------------------------------------------------
# Transforms (same as the tea/cacao/chilli retrain pipelines)
# ---------------------------------------------------------------------------
...
def extract_split_features(feat_sess, feat_input, split_paths, transform,
                           double=False, tag=''):
    feats, labels = [], []
    for cls in TURMERIC_CLASS_NAMES:
        ...
        label = TURMERIC_CLASS_NAMES.index(cls)
        for i in range(0, len(paths), 64):
            ...
            if double:
                ...
                labels.extend([label] * len(chunk))
        print(f'  {tag} {cls:24s} {len(paths)} images', flush=True)
    if not feats:
...
def extract_tta_split(feat_sess, feat_input, split_paths, tag=''):
    ...
    for cls in TURMERIC_CLASS_NAMES:
        ...
        p = sorted([os.path.join(d, f) for f in os.listdir(d)
                    if f.lower().endswith(IMAGE_EXTS)])
        paths.extend(p)
        labels.extend([TURMERIC_CLASS_NAMES.index(cls)] * len(p))
        print(f'  {tag} {cls:24s} {len(p)} images', flush=True)
    if not paths:
...
def extract_tta_train(feat_sess, feat_input, split_paths, double=True):
    ...
    for cls in TURMERIC_CLASS_NAMES:
        ...
        print(f'  train {cls:24s} {len(paths)} imgs x{len(per_img)//len(paths)} views', flush=True)
    return np.vstack(feats), np.array(labels, dtype=np.int64)
...
def metrics_from_cm(cm):
    rows = {}
    for i, cls in enumerate(TURMERIC_CLASS_NAMES):
        tp = cm[i, i]
        fp = cm[:, i].sum() - tp
        fn = cm[i, :].sum() - tp
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    short = [c.split('___')[1] for c in TURMERIC_CLASS_NAMES]
    ...
    try:
        ...
        ax.set_title(f'Turmeric Test Confusion Matrix (acc={100*test_acc:.1f}%)')
    ...
    with open(os.path.join(OUTPUT_DIR, 'confusion_matrix.json'), 'w') as f:
        json.dump({'classes': TURMERIC_CLASS_NAMES, 'confusion_matrix': cm.tolist()},
                  f, indent=4)

    rows, _ = metrics_from_cm(cm)
    lines = ['Turmeric Classification Report (internal test set)', '=' * 60]
...
# ---------------------------------------------------------------------------
# Inference zero-regression spot check on existing non-Turmeric crops
# ---------------------------------------------------------------------------
...
def inference_zero_regression_check(candidate_path, n_existing=123):
    print('\n' + '=' * 60)
    print('INFERENCE ZERO-REGRESSION SPOT CHECK (existing non-Turmeric crops)')
    print('=' * 60)
    ...
    print(f'  crops the candidate now labels Turmeric (FP risk): {turmeric_fp}/{len(paths)}')
    result = {
        'samples': len(paths),
        'max_logit_diff_first_123': max_logit_diff,
        'top1_old_class_changed': argmax_old_diff,
        'turmeric_false_positives': turmeric_fp,
    }
    return result
...
def main():
    ...
    print(f'  Head:          Linear(1280 -> {N_NEW}) Turmeric-only, zero-regression')
    ...
    print(f'  New classes:   indices 123-{122 + N_NEW}: {TURMERIC_CLASS_NAMES}')
    print('=' * 60)

    with open(CLASSES_PATH) as f:
        existing_classes = json.load(f)
    n_existing = len(existing_classes)
    assert n_existing == 123, \
        f'Expected 123 (deployed) classes, got {n_existing}'
    assert not any(c.startswith('Turmeric___') for c in existing_classes), \
        'Turmeric classes already present!'
    candidate_classes = existing_classes + TURMERIC_CLASS_NAMES
    turmeric_start = n_existing  # 123
    turmeric_end = turmeric_start + N_NEW  # 127
    print(f'\n  Existing classes: {n_existing} | Turmeric range: {turmeric_start}-{turmeric_end - 1} | Candidate total: {len(candidate_classes)}')
    ...
    print(f'  Centered Turmeric biases: mean bias {bias_mean:.6f} subtracted')
    ...
    short = [c.split('___')[1][:12] for c in TURMERIC_CLASS_NAMES]
    ...
    for cls in TURMERIC_CLASS_NAMES:
        r = rows[cls]
        print(f'  {cls:28s} prec={r["precision"]:.3f} rec={r["recall"]:.3f} f1={r["f1"]:.3f}')
    ...
    if not args.no_export:
        print('\n' + '=' * 60)
        print('CANDIDATE ONNX (Turmeric rows appended, existing rows frozen)')
        print('=' * 60)
        ...
        if old_weight.shape[0] == 127:
            # Deployed model already has a Turmeric block at 123-126: splice over it
            new_weight = old_weight.copy()
            new_weight[turmeric_start:turmeric_end] = trained_weight
            new_bias = old_bias.copy()
            new_bias[turmeric_start:turmeric_end] = trained_bias
        else:
            # Deployed model (123 classes): append the Turmeric rows
        ...
        candidate_path = os.path.join(OUTPUT_DIR, 'turmeric_candidate.onnx')
        ...
        non_turmeric = list(range(0, turmeric_start))
        w_diff = float(np.max(np.abs(new_weight[non_turmeric] - v1_w[non_turmeric])))
        b_diff = float(np.max(np.abs(new_bias[non_turmeric] - v1_b[non_turmeric])))
        ok = w_diff == 0.0 and b_diff == 0.0
        print(f'  Non-Turmeric weight max diff: {w_diff:.10f}')
        print(f'  Non-Turmeric bias max diff:   {b_diff:.10f}')
        print(f'  ZERO REGRESSION: {"OK" if ok else "FAILED"}')

        inference_result = inference_zero_regression_check(candidate_path, n_existing=123)

        with open(os.path.join(OUTPUT_DIR, 'zero_regression_check.json'), 'w') as f:
            json.dump({'non_turmeric_weight_max_diff': w_diff,
                       'non_turmeric_bias_max_diff': b_diff,
