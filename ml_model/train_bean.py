"""
Train the bean leaf classifier head with a strict zero-regression approach.

Approach (approved, mirrors black pepper/turmeric): freeze all 134 existing
classifier rows, train ONLY the 4 new Bean rows (indices 134-137) on the
prepared Bean crops, then splice the trained rows into a candidate ONNX.
Existing rows remain byte-identical.

Pipeline:
   1. Prepared splits: `dataset/bean/Bean_Split` (train/valid/test,
   ...
   4. Append the trained rows to a candidate ONNX (138 classes) + candidate
      class names. Production model and class_names.json are NOT modified.

Verification:
   * zero_regression_check.json  - byte-identical non-bean rows.
   * inference regression spot check on existing non-bean crops (logits for
     the first 134 classes must be identical; report any bean FPs).

Outputs (`ml_model/bean_retrain_output/`):
   * bean_candidate.onnx + candidate_class_names.json
...
Usage:
    python ml_model/train_bean.py [--tta] [--seed N]
"""
import os
import sys
import json
...
BEAN_CLASS_NAMES = [
    ...
    'bean___downy_mildew',
    'bean___leafminer',
    'bean___powdery_mildew',
]
N_NEW = len(BEAN_CLASS_NAMES)
...
def extract_tta_split(feat_sess, feat_input, split_paths, tag=''):
    ...
    for cls in BEAN_CLASS_NAMES:
        ...
        paths.extend(p)
        labels.extend([BEAN_CLASS_NAMES.index(cls)] * len(p))
...
def extract_tta_train(feat_sess, feat_input, split_paths, double=True):
    ...
    for cls, paths, vpi in per_class:
        label = BEAN_CLASS_NAMES.index(cls)
        skipped = 0
...
def metrics_from_cm(cm):
    rows = {}
    for i, cls in enumerate(BEAN_CLASS_NAMES):
        tp = cm[i, i]
        fp = cm[:, i].sum() - tp
        fn = cm[i, :].sum() - tp
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    short = [c.split('___')[1] for c in BEAN_CLASS_NAMES]
    ...
    try:
        ...
        ax.set_title(f'Bean Test Confusion Matrix (acc={100*test_acc:.1f}%)')
    ...
    with open(os.path.join(OUTPUT_DIR, 'confusion_matrix.json'), 'w') as f:
        json.dump({'classes': BEAN_CLASS_NAMES, 'confusion_matrix': cm.tolist()},
                  f, indent=4)

    rows, _ = metrics_from_cm(cm)
    lines = ['bean Classification Report (internal test set)', '=' * 60]
...
# ---------------------------------------------------------------------------
# Inference zero-regression spot check on existing non-bean crops
# ---------------------------------------------------------------------------
...
def inference_zero_regression_check(candidate_path, n_existing=134):
    print('\n' + '=' * 60)
    print('INFERENCE ZERO-REGRESSION SPOT CHECK (existing non-bean crops)')
    print('=' * 60)
    ...
    print(f'  crops the candidate now labels bean (FP risk): {bean_fp}/{len(paths)}')
    result = {
        'samples': len(paths),
        'max_logit_diff_first_134': max_logit_diff,
        'top1_old_class_changed': argmax_old_diff,
        'bean_false_positives': bean_fp,
    }
    return result
...
def main():
    ...
    print(f'  Head:          Linear(1280 -> {N_NEW}) bean-only, zero-regression')
    ...
    print(f'  New classes:   indices 134-{133 + N_NEW}: {BEAN_CLASS_NAMES}')
    print('=' * 60)

    with open(CLASSES_PATH) as f:
        existing_classes = json.load(f)
    n_existing = len(existing_classes)
    assert n_existing == 134, \
        f'Expected 134 (deployed) classes, got {n_existing}'
    assert not any(c.startswith('bean___') for c in existing_classes), \
        'bean classes already present!'
    candidate_classes = existing_classes + BEAN_CLASS_NAMES
    bean_start = n_existing  # 134
    bean_end = bean_start + N_NEW  # 138
    print(f'\n  Existing classes: {n_existing} | bean range: {bean_start}-{bean_end - 1} | Candidate total: {len(candidate_classes)}')
    ...
    print(f'  Centered bean biases: mean bias {bias_mean:.6f} subtracted')
    ...
    short = [c.split('___')[1][:12] for c in BEAN_CLASS_NAMES]
    ...
    if not args.no_export:
        print('\n' + '=' * 60)
        print('CANDIDATE ONNX (bean rows appended, existing rows frozen)')
        print('=' * 60)
        ...
        assert old_weight.shape[0] == bean_start, \
            f'Expected {bean_start} deployed rows, got {old_weight.shape[0]}'
        ...
        candidate_path = os.path.join(OUTPUT_DIR, 'bean_candidate.onnx')
        ...
        non_bean = list(range(0, bean_start))
        w_diff = float(np.max(np.abs(new_weight[non_bean] - v1_w[non_bean])))
        b_diff = float(np.max(np.abs(new_bias[non_bean] - v1_b[non_bean])))
        ok = w_diff == 0.0 and b_diff == 0.0
        print(f'  Non-bean weight max diff: {w_diff:.10f}')
        print(f'  Non-bean bias max diff:   {b_diff:.10f}')
        print(f'  ZERO REGRESSION: {"OK" if ok else "FAILED"}')

        inference_result = inference_zero_regression_check(candidate_path, n_existing=134)

        with open(os.path.join(OUTPUT_DIR, 'zero_regression_check.json'), 'w') as f:
            json.dump({'non_bean_weight_max_diff': w_diff,
                       'non_bean_bias_max_diff': b_diff,
