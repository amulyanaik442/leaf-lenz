"""
Train the okra leaf classifier head with a strict zero-regression approach.

Approach (approved, mirrors bean/cauliflower/papaya): freeze all 141 existing
classifier rows, train ONLY the 6 new Okra rows (indices 141-146) on the
prepared Okra crops, then splice the trained rows into a candidate ONNX.
Existing rows remain byte-identical.

Pipeline:
   1. Prepared splits: `dataset/okra/Okra_Split` (train/valid/test,
      file-level stratified, dedup, balanced, train-augmented, 80/10/10).
   2. Extract 1280-d features from the frozen production ONNX backbone.
   3. Train a standalone Linear(1280 -> 6) head.
   4. Append the trained rows to a candidate ONNX (152 classes) + candidate
      class names. Production model and class_names.json are NOT modified.

Verification:
   * zero_regression_check.json  - byte-identical non-okra rows.
   * inference regression spot check on existing non-okra crops (logits for
     the first 141 classes must be identical; report any okra FPs).

Outputs (`ml_model/okra_retrain_output/`):
   * okra_candidate.onnx + candidate_class_names.json
...
Usage:
    python ml_model/train_okra.py [--tta] [--seed N]
"""
import os
import sys
import json
...
SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'okra', 'Okra_Split')
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'okra_retrain_output')
CKPT_DIR = os.path.join(OUTPUT_DIR, 'checkpoints')
FEAT_CACHE_TTA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'okra_features_tta.pkl')

# ---------------------------------------------------------------------------
# Config
# ---------------------------------------------------------------------------
OKRA_CLASS_NAMES = [
    'okra___alternaria_leaf_spot',
    'okra___cercospora_leaf_spot',
    'okra___downy_mildew',
    'okra___healthy',
    'okra___leaf_curl_virus',
    'okra___phyllosticta_leaf_spot',
]
N_NEW = len(OKRA_CLASS_NAMES)
...
# ---------------------------------------------------------------------------
# Transforms (same as the tea/cacao/chilli/turmeric/black pepper/bean/papaya
# pipelines)
# ---------------------------------------------------------------------------
...
def extract_tta_split(feat_sess, feat_input, split_paths, tag=''):
    ...
    for cls in OKRA_CLASS_NAMES:
        ...
        paths.extend(p)
        labels.extend([OKRA_CLASS_NAMES.index(cls)] * len(p))
...
def extract_tta_train(feat_sess, feat_input, split_paths, double=True):
    ...
    for cls, paths, vpi in per_class:
        label = OKRA_CLASS_NAMES.index(cls)
        skipped = 0
...
def metrics_from_cm(cm):
    rows = {}
    for i, cls in enumerate(OKRA_CLASS_NAMES):
        tp = cm[i, i]
        fp = cm[:, i].sum() - tp
        fn = cm[i, :].sum() - tp
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    short = [c.split('___')[1] for c in OKRA_CLASS_NAMES]
    ...
    try:
        ...
        ax.set_title(f'Okra Test Confusion Matrix (acc={100*test_acc:.1f}%)')
    ...
    with open(os.path.join(OUTPUT_DIR, 'confusion_matrix.json'), 'w') as f:
        json.dump({'classes': OKRA_CLASS_NAMES, 'confusion_matrix': cm.tolist()},
                  f, indent=4)

    rows, _ = metrics_from_cm(cm)
    lines = ['okra Classification Report (internal test set)', '=' * 60]
...
# ---------------------------------------------------------------------------
# Inference zero-regression spot check on existing non-okra crops
# ---------------------------------------------------------------------------
def collect_existing_crops(max_per_folder=40):
    roots = [
        ...
        os.path.join(BASE_DIR, 'dataset', 'papaya', 'Papaya_Split', 'test'),
    ]
    paths = []
    for root in roots:
...
def inference_zero_regression_check(candidate_path, n_existing=146):
    print('\n' + '=' * 60)
    print('INFERENCE ZERO-REGRESSION SPOT CHECK (existing non-okra crops)')
    print('=' * 60)
    ...
    print(f'  crops the candidate now labels okra (FP risk): {okra_fp}/{len(paths)}')
    result = {
        'samples': len(paths),
        'max_logit_diff_first_146': max_logit_diff,
        'top1_old_class_changed': argmax_old_diff,
        'okra_false_positives': okra_fp,
    }
    return result
...
def main():
    ...
    parser.add_argument('--split-dir', default='Okra_Split',
                        help='split folder under dataset/okra/ (default: Okra_Split)')
    ...
    if args.split_dir != 'Okra_Split':
        SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'okra', args.split_dir)
    split_tag = os.path.basename(SPLIT_DIR)
    FEAT_CACHE_TTA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                  f'okra_features_{split_tag}.pkl')
    ...
    print(f'  Head:          Linear(1280 -> {N_NEW}) okra-only, zero-regression')
    ...
    print(f'  New classes:   {OKRA_CLASS_NAMES}')
    print('=' * 60)

    with open(CLASSES_PATH) as f:
        existing_classes = json.load(f)
    n_existing = len(existing_classes)
    assert n_existing == 146, \
        f'Expected 146 (deployed) classes, got {n_existing}'
    assert not any(c.startswith('okra___') for c in existing_classes), \
        'okra classes already present!'
    candidate_classes = existing_classes + OKRA_CLASS_NAMES
    okra_start = n_existing  # 146
    okra_end = okra_start + N_NEW  # 152
    print(f'\n  Existing classes: {n_existing} | okra range: {okra_start}-{okra_end - 1} | Candidate total: {len(candidate_classes)}')
    ...
    print(f'  Centered okra biases: mean bias {bias_mean:.6f} subtracted')
    ...
    short = [c.split('___')[1][:12] for c in OKRA_CLASS_NAMES]
    ...
    if not args.no_export:
        print('\n' + '=' * 60)
        print('CANDIDATE ONNX (okra rows appended, existing rows frozen)')
        print('=' * 60)
        ...
        assert old_weight.shape[0] == okra_start, \
            f'Expected {okra_start} deployed rows, got {old_weight.shape[0]}'
        ...
        candidate_path = os.path.join(OUTPUT_DIR, 'okra_candidate.onnx')
        ...
        non_okra = list(range(0, okra_start))
        w_diff = float(np.max(np.abs(new_weight[non_okra] - v1_w[non_okra])))
        b_diff = float(np.max(np.abs(new_bias[non_okra] - v1_b[non_okra])))
        ok = w_diff == 0.0 and b_diff == 0.0
        print(f'  Non-okra weight max diff: {w_diff:.10f}')
        print(f'  Non-okra bias max diff:   {b_diff:.10f}')
        print(f'  ZERO REGRESSION: {"OK" if ok else "FAILED"}')

        inference_result = inference_zero_regression_check(candidate_path, n_existing=146)

        with open(os.path.join(OUTPUT_DIR, 'zero_regression_check.json'), 'w') as f:
            json.dump({'non_okra_weight_max_diff': w_diff,
                       'non_okra_bias_max_diff': b_diff,
