"""Post-deploy smoke test for the Black Pepper integration.

1. Black Pepper routing: each BlackPepper_Split/test image via TTA predict with
   prefix='BlackPepper___' must return a BlackPepper___ label.
2. Zero regression (crop-filtered): for a sample of existing-crop images, the
   crop-filtered top-1 from the NEW (130-class) model must exactly match the
   PRE-DEPLOY backup (127-class) model.
3. App wiring: predict_by_crop(..., 'black_pepper') routes to the general model
   and returns a BlackPepper___ label; black_pepper crop key is registered.
"""
import os
import sys
import json
...
BACKUP_DIR = sorted(glob.glob(os.path.join(ML_ASSETS, 'backup_pre_blackpepper_*')))[-1]
...
def crop_prefix(crop):
    return {'cacao': 'Cacao___', 'tea': 'Tea___', 'pumpkin': 'Pumpkin___',
            'chilli': 'Chilli___', 'turmeric': 'Turmeric___'}[crop]
...
def main():
    ...
    # ---- 1. black pepper routing ----
    print('\n=== BLACK PEPPER ROUTING (BlackPepper_Split/test, prefix="BlackPepper___") ===')
    test_root = os.path.join(BASE_DIR, 'dataset', 'black_pepper', 'BlackPepper_Split', 'test')
    total = correct = 0
    for cls in sorted(os.listdir(test_root)):
        ...
        ok = 0
        for p in imgs:
            label, conf = tta_predict(new_sess, new_names, p, prefix='BlackPepper___')
            total += 1
            is_corr = (label == cls)
            ok += int(is_corr)
            correct += int(is_corr)
    ...
    print(f'  black pepper test accuracy: {correct}/{total} = {100*correct/total:.1f}%')
    ...
    try:
        ...
        print(f'  black_pepper in CROP_CLASS_PREFIXES: {CROP_CLASS_PREFIXES.get("black_pepper")}')
        sample = None
        for cls in sorted(os.listdir(test_root)):
            d = os.path.join(test_root, cls)
            for f in sorted(os.listdir(d)):
        ...
        assert label.startswith('BlackPepper___'), 'expected a BlackPepper___ label'
