"""Post-deploy smoke test for the Turmeric integration.

1. Turmeric routing: each Turmeric_Split/test image via TTA predict with
   prefix='Turmeric___' must return a Turmeric___ label.
2. Zero regression (crop-filtered): for a sample of existing-crop images, the
   crop-filtered top-1 from the NEW (127-class) model must exactly match the
   PRE-DEPLOY backup (123-class) model.
3. App wiring: predict_by_crop(..., 'turmeric') routes to the general model and
   returns a Turmeric___ label; turmeric crop key is registered.
"""
import os
import sys
import json
...
BACKUP_DIR = sorted(glob.glob(os.path.join(ML_ASSETS, 'backup_pre_turmeric_*')))[-1]
...
def tta_predict(sess, names, img_path, prefix=None):
    ...
    if prefix is not None:
        ...
        z[keep] = probs[keep]
        s = z.sum()
        filtered = z / s if s > 0 else probs
...
def main():
    ...
    # ---- 1. turmeric routing ----
    print('\n=== TURMERIC ROUTING (Turmeric_Split/test, prefix="Turmeric___") ===')
    test_root = os.path.join(BASE_DIR, 'dataset', 'turmeric', 'Turmeric_Split', 'test')
    total = correct = 0
    for cls in sorted(os.listdir(test_root)):
        ...
        ok = 0
        for p in imgs:
            label, conf = tta_predict(new_sess, new_names, p, prefix='Turmeric___')
            total += 1
            is_corr = (label == cls)
            ok += int(is_corr)
            correct += int(is_corr)
    ...
    print(f'  turmeric test accuracy: {correct}/{total} = {100*correct/total:.1f}%')
    ...
    try:
        ...
        print(f'  turmeric in CROP_CLASS_PREFIXES: {CROP_CLASS_PREFIXES.get("turmeric")}')
        sample = None
        for cls in sorted(os.listdir(test_root)):
            d = os.path.join(test_root, cls)
            for f in sorted(os.listdir(d)):
        ...
        with open(sample, 'rb') as fh:
            label, conf, top5, mm = predict_by_crop(fh, 'turmeric')
        print(f'  predict_by_crop(turmeric): {label} ({conf:.3f})')
        assert label.startswith('Turmeric___'), 'expected a Turmeric___ label'
        # specialist models still load
        with open(sample, 'rb') as fh:
