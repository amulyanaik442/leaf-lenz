"""
Train the Cacao leaf classifier head with a strict zero-regression approach.

Approach (approved): freeze all 115 existing classifier rows, train ONLY the 2
new Cacao rows (indices 115-116) on the prepared Cacao crops, then splice the
trained rows into a candidate ONNX. Existing rows remain byte-identical.

Pipeline:
  1. Prepared splits: `dataset/cacao_leaf/Cacao_Split` (train/valid/test,
     blur-filtered, balanced, train-augmented, source-image leakage-free).
  2. Extract 1280-d features from the frozen production ONNX backbone.
  3. Train a standalone Linear(1280 -> 2) head.
  4. Append the trained rows to a candidate ONNX (117 classes) + candidate
     class names.  Production model and class_names.json are NOT modified.

Verification:
  * zero_regression_check.json  - byte-identical non-Cacao rows.
  * inference regression spot check on existing non-Cacao crops (logits for
    the first 115 classes must be identical; report any Cacao false positives).

Outputs (`ml_model/cacao_retrain_output/`):
  * cacao_candidate.onnx + candidate_class_names.json
...
Usage:
    python ml_model/train_cacao_leaf.py
"""
import os
import sys
import json
...
SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'cacao_leaf', 'Cacao_Split')
FEAT_CACHE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'cacao_features.pkl')
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'cacao_retrain_output')
...
CACAO_CLASS_NAMES = [FOLDER_TO_MODEL[f] for f in ('healthy', 'anthracnose')]
...
# ---------------------------------------------------------------------------
# Transforms (same as the tea/pumpkin retrain pipelines)
# ---------------------------------------------------------------------------
...
def extract_split_features(feat_sess, feat_input, split_paths, transform,
    ...
    for folder, model_cls in FOLDER_TO_MODEL.items():
        ...
        label = CACAO_CLASS_NAMES.index(model_cls)
        for i in range(0, len(paths), 64):
            ...
            if double:
                ...
                labels.extend([label] * len(chunk))
        print(f'  {tag} {folder:14s} {len(paths)} images', flush=True)
    if not feats:
...
def metrics_from_cm(cm):
    rows = {}
    for i, cls in enumerate(CACAO_CLASS_NAMES):
        tp = cm[i, i]
        fp = cm[:, i].sum() - tp
        fn = cm[i, :].sum() - tp
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    short = [c.split('___')[1] for c in CACAO_CLASS_NAMES]
    ...
    try:
        ...
        ax.set_xticks(range(2), short)
        ax.set_yticks(range(2), short)
        ax.set_xlabel('Predicted'); ax.set_ylabel('True')
        ax.set_title(f'Cacao Test Confusion Matrix (acc={100*test_acc:.1f}%)')
        for i in range(2):
            for j in range(2):
    ...
    with open(os.path.join(OUTPUT_DIR, 'confusion_matrix.json'), 'w') as f:
        json.dump({'classes': CACAO_CLASS_NAMES, 'confusion_matrix': cm.tolist()},
                  f, indent=4)

    rows, _ = metrics_from_cm(cm)
    lines = ['Cacao Classification Report (internal test set)', '=' * 60]
    lines.append(f'{"class":28s} {"precision":>10s} {"recall":>10s} {"f1":>10s} {"support":>8s}')
    for cls in CACAO_CLASS_NAMES:
        r = rows[cls]
...
# ---------------------------------------------------------------------------
# Inference zero-regression spot check on existing non-Cacao crops
# ---------------------------------------------------------------------------
def collect_existing_crops(max_per_folder=40):
    """Gather a handful of existing (non-Cacao) crops for the regression check."""
    roots = [
...
def inference_zero_regression_check(candidate_path, n_existing=115):
    """Compare production vs candidate logits for the first n_existing classes."""
    print('\n' + '=' * 60)
    print('INFERENCE ZERO-REGRESSION SPOT CHECK (existing non-Cacao crops)')
    print('=' * 60)
    ...
    print(f'  crops the candidate now labels Cacao (FP risk): {cacao_fp}/{len(paths)}')
    result = {
        'samples': len(paths),
        'max_logit_diff_first_115': max_logit_diff,
        'top1_old_class_changed': argmax_old_diff,
        'cacao_false_positives': cacao_fp,
    }
    return result
...
def main():
    ...
    print(f'  Head:          Linear(1280 -> 2) Cacao-only, zero-regression')
    ...
    print(f'  New classes:   indices 115-116: {CACAO_CLASS_NAMES}')
    print('=' * 60)

    with open(CLASSES_PATH) as f:
        existing_classes = json.load(f)
    n_existing = len(existing_classes)
    assert n_existing == 115, f'Expected 115 existing classes, got {n_existing}'
    assert not any(c.startswith('Cacao___') for c in existing_classes), \
        'Cacao classes already present!'
    cacao_start = n_existing  # 115
    cacao_end = cacao_start + 2  # 117
    candidate_classes = existing_classes + CACAO_CLASS_NAMES
    print(f'\n  Existing classes: {n_existing} | Cacao range: {cacao_start}-{cacao_end - 1} | Candidate total: {len(candidate_classes)}')
    ...
    print('=' * 60)
    head = nn.Linear(1280, 2).to(device)
    ...
    print(f'  Centered Cacao biases: mean bias {bias_mean:.6f} subtracted')
    ...
    short = [c.split('___')[1][:12] for c in CACAO_CLASS_NAMES]
    print('  ' + ' '*15 + ''.join(f'{s:>13s}' for s in short))
    for i in range(2):
        print(f'  {short[i]:14s}' + ''.join(f'{cm[i][j]:6d}' for j in range(2)))
    rows, _ = metrics_from_cm(cm)
    for cls in CACAO_CLASS_NAMES:
        r = rows[cls]
    ...
    print('\n' + '=' * 60)
    print('CANDIDATE ONNX (Cacao rows appended, existing rows frozen)')
    print('=' * 60)
    ...
    candidate_path = os.path.join(OUTPUT_DIR, 'cacao_candidate.onnx')
    ...
    non_cacao = list(range(0, cacao_start))
    w_diff = float(np.max(np.abs(new_weight[non_cacao] - v1_w[non_cacao])))
    b_diff = float(np.max(np.abs(new_bias[non_cacao] - v1_b[non_cacao])))
    ok = w_diff == 0.0 and b_diff == 0.0
    print(f'  Non-Cacao weight max diff: {w_diff:.10f}')
    print(f'  Non-Cacao bias max diff:   {b_diff:.10f}')
    print(f'  ZERO REGRESSION: {"OK" if ok else "FAILED"}')

    inference_result = inference_zero_regression_check(candidate_path, n_existing=115)

    with open(os.path.join(OUTPUT_DIR, 'zero_regression_check.json'), 'w') as f:
        json.dump({'non_cacao_weight_max_diff': w_diff,
                   'non_cacao_bias_max_diff': b_diff,
