"""
Train the Black Pepper leaf classifier head with a strict zero-regression approach.

Approach (approved, mirrors chilli/turmeric): freeze all 127 existing classifier
rows, train ONLY the 3 new Black Pepper rows (indices 127-129) on the prepared
Black Pepper crops, then splice the trained rows into a candidate ONNX.
Existing rows remain byte-identical.

Pipeline:
  1. Prepared splits: `dataset/black_pepper/BlackPepper_Split` (train/valid/test,
     deduped, blur-filtered, balanced, train-augmented, stratified 80/10/10 with
     documented near-dup leakage - approved).
  2. Extract 1280-d features from the frozen production ONNX backbone.
  3. Train a standalone Linear(1280 -> 3) head.
  4. Append the trained rows to a candidate ONNX (130 classes) + candidate
     class names. Production model and class_names.json are NOT modified.

Verification:
  * zero_regression_check.json  - byte-identical non-BlackPepper rows.
  * inference regression spot check on existing non-BlackPepper crops (logits for
    the first 127 classes must be identical; report any BlackPepper FPs).

Outputs (`ml_model/black_pepper_retrain_output/`):
  * black_pepper_candidate.onnx + candidate_class_names.json
...
Usage:
    python ml_model/train_black_pepper.py [--tta] [--seed N]
"""
import os
import sys
import json
...
SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'black_pepper', 'BlackPepper_Split')
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'black_pepper_retrain_output')
CKPT_DIR = os.path.join(OUTPUT_DIR, 'checkpoints')
FEAT_CACHE_TTA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'black_pepper_features_tta.pkl')

# ---------------------------------------------------------------------------
# Config
# ---------------------------------------------------------------------------
BLACK_PEPPER_CLASS_NAMES = [
    'BlackPepper___healthy',
    'BlackPepper___leaf_blight',
    'BlackPepper___yellow_mottle_virus',
]
N_NEW = len(BLACK_PEPPER_CLASS_NAMES)
...
# ---------------------------------------------------------------------------
# Transforms (same as the tea/cacao/chilli/turmeric retrain pipelines)
# ---------------------------------------------------------------------------
...
def extract_tta_split(feat_sess, feat_input, split_paths, tag=''):
    paths = []
    labels = []
    for cls in BLACK_PEPPER_CLASS_NAMES:
        ...
        paths.extend(p)
        labels.extend([BLACK_PEPPER_CLASS_NAMES.index(cls)] * len(p))
...
def extract_tta_train(feat_sess, feat_input, split_paths, double=True):
    ...
    for cls in BLACK_PEPPER_CLASS_NAMES:
        ...
        paths = sorted([os.path.join(d, f) for f in os.listdir(d)
                        if f.lower().endswith(IMAGE_EXTS)])
        label = BLACK_PEPPER_CLASS_NAMES.index(cls)
        per_img = []
        for p in paths:
            t = tform(Image.open(p).convert('RGB')).numpy()
        ...
        print(f'  train {cls:30s} {len(paths)} imgs x{len(per_img)//len(paths)} views', flush=True)
...
def metrics_from_cm(cm):
    rows = {}
    for i, cls in enumerate(BLACK_PEPPER_CLASS_NAMES):
        tp = cm[i, i]
        fp = cm[:, i].sum() - tp
        fn = cm[i, :].sum() - tp
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    short = [c.split('___')[1] for c in BLACK_PEPPER_CLASS_NAMES]
    ...
    try:
        ...
        ax.set_title(f'Black Pepper Test Confusion Matrix (acc={100*test_acc:.1f}%)')
    ...
    with open(os.path.join(OUTPUT_DIR, 'confusion_matrix.json'), 'w') as f:
        json.dump({'classes': BLACK_PEPPER_CLASS_NAMES, 'confusion_matrix': cm.tolist()},
                  f, indent=4)

    rows, _ = metrics_from_cm(cm)
    lines = ['Black Pepper Classification Report (internal test set)', '=' * 60]
...
# ---------------------------------------------------------------------------
# Inference zero-regression spot check on existing non-BlackPepper crops
# ---------------------------------------------------------------------------
...
def inference_zero_regression_check(candidate_path, n_existing=127):
    print('\n' + '=' * 60)
    print('INFERENCE ZERO-REGRESSION SPOT CHECK (existing non-BlackPepper crops)')
    print('=' * 60)
    ...
    print(f'  crops the candidate now labels BlackPepper (FP risk): {black_pepper_fp}/{len(paths)}')
    result = {
        'samples': len(paths),
        'max_logit_diff_first_127': max_logit_diff,
        'top1_old_class_changed': argmax_old_diff,
        'black_pepper_false_positives': black_pepper_fp,
    }
    return result
...
def main():
    ...
    print(f'  Head:          Linear(1280 -> {N_NEW}) BlackPepper-only, zero-regression')
    ...
    print(f'  New classes:   indices 127-{126 + N_NEW}: {BLACK_PEPPER_CLASS_NAMES}')
    print('=' * 60)

    with open(CLASSES_PATH) as f:
        existing_classes = json.load(f)
    n_existing = len(existing_classes)
    assert n_existing == 127, \
        f'Expected 127 (deployed) classes, got {n_existing}'
    assert not any(c.startswith('BlackPepper___') for c in existing_classes), \
        'BlackPepper classes already present!'
    candidate_classes = existing_classes + BLACK_PEPPER_CLASS_NAMES
    pepper_start = n_existing  # 127
    pepper_end = pepper_start + N_NEW  # 130
    print(f'\n  Existing classes: {n_existing} | BlackPepper range: {pepper_start}-{pepper_end - 1} | Candidate total: {len(candidate_classes)}')
    ...
    print(f'  Centered BlackPepper biases: mean bias {bias_mean:.6f} subtracted')
    ...
    short = [c.split('___')[1][:12] for c in BLACK_PEPPER_CLASS_NAMES]
    ...
    if not args.no_export:
        print('\n' + '=' * 60)
        print('CANDIDATE ONNX (BlackPepper rows appended, existing rows frozen)')
        print('=' * 60)
        ...
        assert old_weight.shape[0] == pepper_start, \
            f'Expected {pepper_start} deployed rows, got {old_weight.shape[0]}'
        ...
        candidate_path = os.path.join(OUTPUT_DIR, 'black_pepper_candidate.onnx')
        ...
        non_pepper = list(range(0, pepper_start))
        w_diff = float(np.max(np.abs(new_weight[non_pepper] - v1_w[non_pepper])))
        b_diff = float(np.max(np.abs(new_bias[non_pepper] - v1_b[non_pepper])))
        ok = w_diff == 0.0 and b_diff == 0.0
        print(f'  Non-BlackPepper weight max diff: {w_diff:.10f}')
        print(f'  Non-BlackPepper bias max diff:   {b_diff:.10f}')
        print(f'  ZERO REGRESSION: {"OK" if ok else "FAILED"}')

        inference_result = inference_zero_regression_check(candidate_path, n_existing=127)

        with open(os.path.join(OUTPUT_DIR, 'zero_regression_check.json'), 'w') as f:
            json.dump({'non_black_pepper_weight_max_diff': w_diff,
                       'non_black_pepper_bias_max_diff': b_diff,
