"""
Train the Tea classifier head (v2) with a strict zero-regression approach.

v2 changes (approved plan):
  * brown blight + gray light merged -> Tea___blight (7 Tea classes)
  * training set now includes independent tealeafBD images (domain coverage)
  * candidate appends 7 rows at indices 108-114 -> 115 classes total

Same zero-regression guarantee: all 108 existing classifier rows frozen and
byte-identical in the candidate. Production model and class_names.json are
NOT modified.

Outputs (`ml_model/tea_retrain_output_v2/`):
  * tea_candidate.onnx + candidate_class_names.json
  * best_model.pth + checkpoints/epoch_NN.pth
  * training_history.json, training_curves.png, confusion_matrix.*
  * classification_report.*, final_accuracies.json, zero_regression_check.json

Usage:
    python ml_model/train_tea_v2.py
"""
import os
import sys
import json
...
NUM_CLASSES = len(TEA_CLASS_NAMES)  # 7
    ...
    for folder, model_cls in FOLDER_TO_MODEL.items():
        ...
        for i in range(0, len(paths), 64):
            ...
            if double:
                ...
                labels.extend([label] * len(chunk))
        print(f'  {tag} {folder:16s} {len(paths)} images', flush=True)
    if not feats:
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    try:
        ...
        ax.set_xticks(range(NUM_CLASSES), short, rotation=30, ha='right')
        ax.set_yticks(range(NUM_CLASSES), short)
...
def main():
    ...
    print(f'  Head:          Linear(1280 -> {NUM_CLASSES}) Tea-only, zero-regression')
    ...
    print(f'  New classes:   indices 108-{107 + NUM_CLASSES}: {TEA_CLASS_NAMES}')
    print('=' * 60)
    ...
    tea_end = tea_start + NUM_CLASSES  # 115
    ...
    for i in range(NUM_CLASSES):
        print(f'  {short[i]:14s}' + ''.join(f'{cm[i][j]:6d}' for j in range(NUM_CLASSES)))
    rows, _ = metrics_from_cm(cm)
    for cls in TEA_CLASS_NAMES:
        r = rows[cls]
