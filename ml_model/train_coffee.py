"""
Train the coffee leaf classifier head with a strict zero-regression approach.

Approach (mirrors okra-v2/papaya-v2): freeze all 149 existing non-coffee
classifier rows, train ONLY the 3 Coffee rows (indices 17-19) on the prepared
RoCoLe Coffee crops, then splice the trained rows into a candidate ONNX in
place. Existing rows remain byte-identical.

Pipeline:
   1. Prepared splits: `dataset/coffee/Coffee_Split` (train/valid/test,
      file-level stratified, 80/10/10, dedup, seed 42).
   2. Extract 1280-d features from the frozen production ONNX backbone
      (TTA-aligned: 4 flip views for eval, 6 views for train).
   3. Train a standalone Linear(1280 -> 3) head (weight-space ensemble over
      seeds, label smoothing, weight decay).
   4. Replace rows 17-19 in a candidate ONNX (152 classes) + candidate class
      names. Production model and class_names.json are NOT modified.

Verification:
   * zero_regression_check.json - byte-identical non-coffee rows.
   * inference regression spot check on existing non-coffee crops.

Outputs (`ml_model/coffee_retrain_output/`):
   * coffee_candidate.onnx + candidate_class_names.json
   * best_model.pth + checkpoints/seed{seed}_epoch_NN.pth
...
Usage:
    python ml_model/train_coffee.py [--tta] [--seed N] [--seeds 42,7,123]
                                    [--wd 0.01 --smoothing 0.2]
"""
import os
import sys
import json
...
SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'coffee', 'Coffee_Split')
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'coffee_retrain_output')
CKPT_DIR = os.path.join(OUTPUT_DIR, 'checkpoints')
FEAT_CACHE_TTA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'coffee_features_tta.pkl')
...
# Split folders on disk are lowercase; model class names use a capital C.
COFFEE_FOLDER_NAMES = [
    'coffee___healthy',
    'coffee___red_spider_mite',
    'coffee___rust',
]
FOLDER_TO_CLASS = dict(zip(COFFEE_FOLDER_NAMES, COFFEE_CLASS_NAMES))
N_NEW = len(COFFEE_CLASS_NAMES)
...
# ---------------------------------------------------------------------------
# Transforms (same as the papaya/okra pipelines)
# ---------------------------------------------------------------------------
...
def _class_folder(cls, folder):
    return folder if folder in FOLDER_TO_CLASS else \
        next((f for f in COFFEE_FOLDER_NAMES if f.startswith(cls.split('___')[1])), None)


def extract_tta_split(feat_sess, feat_input, split_paths, tag=''):
    paths = []
    labels = []
    for i, cls in enumerate(COFFEE_CLASS_NAMES):
        d = os.path.join(split_paths, COFFEE_FOLDER_NAMES[i])
...
def extract_tta_train(feat_sess, feat_input, split_paths, double=True):
    ...
    for i, paths, vpi in per_class:
        ...
        for w in range(0, len(paths), FEAT_WINDOW):
            ...
            if not views:
                continue
            labels[cursor:cursor + len(views)] = i
            for k in range(0, len(views), FEAT_BATCH):
                arr = np.stack(views[k:k + FEAT_BATCH]).astype(np.float32)
                out = feat_sess.run(None, {feat_input: arr})[-1]
                train_feats[cursor + k:cursor + k + out.shape[0]] = out
            cursor += len(views)
            del views
        if skipped:
            print(f'  WARNING: train {COFFEE_FOLDER_NAMES[i]} skipped {skipped} unreadable images', flush=True)
        n_views = vpi * len(paths) - skipped * vpi if skipped else vpi * len(paths)
        print(f'  train {COFFEE_FOLDER_NAMES[i]:30s} {len(paths)} imgs x{vpi} views = {n_views}', flush=True)
...
def class_weights_from_labels(labels):
    n = len(labels)
    counts = np.bincount(labels, minlength=N_NEW).astype(np.float64)
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    short = [c.split('___')[1] for c in COFFEE_CLASS_NAMES]
    ...
    try:
        ...
        ax.set_title(f'Coffee Test Confusion Matrix (acc={100*test_acc:.1f}%)')
        for i in range(N_NEW):
    ...
    with open(os.path.join(OUTPUT_DIR, 'confusion_matrix.json'), 'w') as f:
        json.dump({'classes': COFFEE_CLASS_NAMES, 'confusion_matrix': cm.tolist()},
                  f, indent=4)

    rows, _ = metrics_from_cm(cm)
    lines = ['coffee Classification Report (internal test set)', '=' * 60]
    lines.append(f'{"class":30s} {"precision":>10s} {"recall":>10s} {"f1":>10s} {"support":>8s}')
    for cls in COFFEE_CLASS_NAMES:
        r = rows[cls]
...
# ---------------------------------------------------------------------------
# Inference zero-regression spot check on existing non-coffee crops
# ---------------------------------------------------------------------------
def collect_existing_crops(max_per_folder=40):
    roots = [
        ...
        os.path.join(BASE_DIR, 'dataset', 'okra', 'Okra_Split_full', 'test'),
        os.path.join(BASE_DIR, 'dataset', 'papaya', 'Papaya_Split_full', 'test'),
    ]
    paths = []
    for root in roots:
...
def inference_zero_regression_check(candidate_path, coffee_range=(17, 20)):
    print('\n' + '=' * 60)
    print('INFERENCE ZERO-REGRESSION SPOT CHECK (existing non-coffee crops)')
    print('=' * 60)
    ...
    protected = [i for i in range(cand.get_outputs()[0].shape[1])
                 if not (coffee_range[0] <= i < coffee_range[1])]
    max_logit_diff = 0.0
    argmax_old_diff = 0
    coffee_fp = 0
    ...
    for p in paths:
        ...
        if np.argmax(lp[protected]) != np.argmax(lc[protected]):
            argmax_old_diff += 1
        if coffee_range[0] <= np.argmax(lc) < coffee_range[1]:
            coffee_fp += 1

    print(f'  max |logit diff| over protected (non-coffee) classes: {max_logit_diff:.2e}')
    print(f'  crops whose top-1 among protected classes changed: {argmax_old_diff}/{len(paths)}')
    print(f'  crops the candidate now labels coffee (FP risk): {coffee_fp}/{len(paths)}')
    result = {
        'samples': len(paths),
        'max_logit_diff_protected': max_logit_diff,
        'protected_index_count': len(protected),
        'top1_old_class_changed': argmax_old_diff,
        'coffee_false_positives': coffee_fp,
    }
    return result
...
def main():
    ...
    parser.add_argument('--class-weights', action='store_true',
                        help='weight the loss by inverse class frequency '
                             '(helps the underrepresented red_spider_mite class)')
    parser.add_argument('--seed', type=int, default=SEED)
    parser.add_argument('--seeds', type=str, default=None,
                        help='comma-separated seeds to train and average '
                             '(linear head weight-space ensemble; overrides --seed)')
    parser.add_argument('--split-dir', default='Coffee_Split',
                        help='split folder under dataset/coffee/ (default: Coffee_Split)')
    ...
    if args.split_dir != 'Coffee_Split':
        SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'coffee', args.split_dir)
    split_tag = os.path.basename(SPLIT_DIR)
    FEAT_CACHE_TTA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                  f'coffee_features_{split_tag}.pkl')
    ...
    print(f'  Head:          Linear(1280 -> {N_NEW}) coffee-only, zero-regression')
    print(f'  Loss:          CrossEntropyLoss(label_smoothing=0.1'
          f'{", class-weighted" if args.class_weights else ""})')
    print(f'  Optimizer:     Adam lr={LR}, wd={args.wd}')
    ...
    print(f'  Seeds:         {seed_list} ({"ensemble" if ensemble else "single"})')
    print(f'  New classes:   {COFFEE_CLASS_NAMES}')
    print('=' * 60)

    with open(CLASSES_PATH) as f:
        existing_classes = json.load(f)
    n_existing = len(existing_classes)
    first_coffee = next((i for i, c in enumerate(existing_classes)
                         if c.startswith('Coffee___')), None)
    assert first_coffee is not None, 'reference model has no Coffee rows'
    assert existing_classes[first_coffee:first_coffee + N_NEW] == COFFEE_CLASS_NAMES, \
        'coffee row order mismatch in reference model'
    coffee_start = first_coffee
    candidate_classes = existing_classes
    coffee_end = coffee_start + N_NEW
    print(f'\n  Existing classes: {n_existing} | coffee range: '
          f'{coffee_start}-{coffee_end - 1} (REPLACE IN PLACE) | '
          f'Candidate total: {len(candidate_classes)}')
    ...
    if args.class_weights:
        cw = class_weights_from_labels(train_labels).to(device)
    ...
    print(f'  Centered coffee biases: mean bias {bias_mean:.6f} subtracted')
    ...
    short = [c.split('___')[1][:14] for c in COFFEE_CLASS_NAMES]
    print('  ' + ' '*17 + ''.join(f'{s:>17s}' for s in short))
    for i in range(N_NEW):
        print(f'  {short[i]:16s}' + ''.join(f'{cm[i][j]:6d}' for j in range(N_NEW)))
    rows, _ = metrics_from_cm(cm)
    for cls in COFFEE_CLASS_NAMES:
        r = rows[cls]
    ...
    _fa['split'] = split_tag
    _fa['class_weights'] = bool(args.class_weights)
    with open(os.path.join(OUTPUT_DIR, 'final_accuracies.json'), 'w') as _f:
        json.dump(_fa, _f, indent=4)

    # ---- Candidate ONNX (replace coffee rows in place, zero regression) ----
    candidate_path = None
    w_diff = b_diff = None
    ok = None
    ...
    if not args.no_export:
        print('\n' + '=' * 60)
        print('CANDIDATE ONNX (coffee rows replaced, other rows frozen)')
        print('=' * 60)
        ...
        new_weight[coffee_start:coffee_end] = trained_weight
        new_bias[coffee_start:coffee_end] = trained_bias
        ...
        candidate_path = os.path.join(OUTPUT_DIR, 'coffee_candidate.onnx')
        ...
        # Zero-regression verification (non-coffee rows byte-identical)
        ...
        non_coffee = [i for i in range(new_weight.shape[0])
                      if not (coffee_start <= i < coffee_end)]
        w_diff = float(np.max(np.abs(new_weight[non_coffee] - v1_w[non_coffee])))
        b_diff = float(np.max(np.abs(new_bias[non_coffee] - v1_b[non_coffee])))
        ok = w_diff == 0.0 and b_diff == 0.0
        print(f'  Non-coffee weight max diff: {w_diff:.10f}')
        print(f'  Non-coffee bias max diff:   {b_diff:.10f}')
        print(f'  ZERO REGRESSION: {"OK" if ok else "FAILED"}')

        inference_result = inference_zero_regression_check(
            candidate_path, coffee_range=(coffee_start, coffee_end))

        with open(os.path.join(OUTPUT_DIR, 'zero_regression_check.json'), 'w') as f:
            json.dump({'non_coffee_weight_max_diff': w_diff,
                       'non_coffee_bias_max_diff': b_diff,
