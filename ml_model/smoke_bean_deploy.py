"""Post-deploy smoke test for the Bean integration.

1. Bean routing: each Bean_Split/test image via TTA predict with
   prefix='bean___' must return a bean___ label.
2. Zero regression (crop-filtered): for a sample of existing-crop images, the
   ...
   PRE-DEPLOY backup (134-class) model.
3. App wiring: predict_by_crop(..., 'bean') routes to the general model and
   returns a bean___ label; bean crop key is registered.
"""
import os
import sys
import json
...
BACKUP_DIR = sorted(glob.glob(os.path.join(ML_ASSETS, 'backup_pre_bean_*')))[-1]
...
def crop_prefix(crop):
    return {'cacao': 'Cacao___', 'tea': 'Tea___', 'pumpkin': 'Pumpkin___',
            ...
            'black_pepper': 'BlackPepper___', 'ginger': 'ginger___'}[crop]
...
def main():
    ...
    # ---- 1. bean routing ----
    print('\n=== BEAN ROUTING (Bean_Split/test, prefix="bean___") ===')
    test_root = os.path.join(BASE_DIR, 'dataset', 'bean', 'Bean_Split', 'test')
    total = correct = 0
    for cls in sorted(os.listdir(test_root)):
        ...
        ok = 0
        for p in imgs:
            label, conf = tta_predict(new_sess, new_names, p, prefix='bean___')
            total += 1
            is_corr = (label == cls)
            ok += int(is_corr)
            correct += int(is_corr)
    ...
    print(f'  bean test accuracy: {correct}/{total} = {100*correct/total:.1f}%')
    ...
    try:
        ...
        print(f'  bean in CROP_CLASS_PREFIXES: {CROP_CLASS_PREFIXES.get("bean")}')
        sample = None
        for cls in sorted(os.listdir(test_root)):
            d = os.path.join(test_root, cls)
            for f in sorted(os.listdir(d)):
        ...
        assert label.startswith('bean___'), 'expected a bean___ label'
        with open(sample, 'rb') as fh:
