"""
Train the cauliflower leaf classifier head with a strict zero-regression approach.

Approach (approved, mirrors black pepper/turmeric/bean): freeze all existing
classifier rows, train ONLY the cauliflower rows on the prepared Cauliflower
crops, then splice the trained rows into a candidate ONNX. Existing rows
remain byte-identical.

Note: the deployed model now has 141 classes; cauliflower occupies indices
138-140 (healthy, black_rot, downy_mildew). The bacterial_spot_rot class was
removed from the dataset (2026-08-04) and dropped from the model, so the head
trains 3 classes at indices 138-140 (141 total).

Pipeline:
    1. Prepared splits: `dataset/cauliflower/Cauliflower_Split` (train/valid/test,
       file-level stratified, no blur filter, balanced, train-augmented,
       80/10/10 - approved).
    2. Extract 1280-d features from the frozen production ONNX backbone.
    3. Train a standalone Linear(1280 -> 3) head.
    4. Append the trained rows to a candidate ONNX (141 classes) + candidate
       class names. Production model and class_names.json are NOT modified.

Verification:
   * zero_regression_check.json  - byte-identical non-cauliflower rows.
   * inference regression spot check on existing non-cauliflower crops (logits
     for the first 138 classes must be identical; report any cauliflower FPs).

Outputs (`ml_model/cauliflower_retrain_output/`):
   * cauliflower_candidate.onnx + candidate_class_names.json
...
Usage:
    python ml_model/train_cauliflower.py [--tta] [--seed N]
"""
import os
import sys
import json
...
SPLIT_DIR = os.path.join(BASE_DIR, 'dataset', 'cauliflower', 'Cauliflower_Split')
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'cauliflower_retrain_output')
CKPT_DIR = os.path.join(OUTPUT_DIR, 'checkpoints')
FEAT_CACHE_TTA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'cauliflower_features_tta.pkl')

# ---------------------------------------------------------------------------
# Config
# ---------------------------------------------------------------------------
CAULIFLOWER_CLASS_NAMES = [
    'cauliflower___healthy',
    'cauliflower___black_rot',
    'cauliflower___downy_mildew',
]
N_NEW = len(CAULIFLOWER_CLASS_NAMES)
...
# ---------------------------------------------------------------------------
# Transforms (same as the bean/turmeric/black pepper pipelines)
# ---------------------------------------------------------------------------
...
def extract_tta_split(feat_sess, feat_input, split_paths, tag=''):
    paths = []
    labels = []
    for cls in CAULIFLOWER_CLASS_NAMES:
        ...
        paths.extend(p)
        labels.extend([CAULIFLOWER_CLASS_NAMES.index(cls)] * len(p))
...
def extract_tta_train(feat_sess, feat_input, split_paths, double=True):
    ...
    for cls, paths, vpi in per_class:
        label = CAULIFLOWER_CLASS_NAMES.index(cls)
        skipped = 0
...
def metrics_from_cm(cm):
    rows = {}
    for i, cls in enumerate(CAULIFLOWER_CLASS_NAMES):
        tp = cm[i, i]
        fp = cm[:, i].sum() - tp
        fn = cm[i, :].sum() - tp
...
def save_artifacts(history, cm, test_acc, train_acc, best_val_acc, device):
    ...
    short = [c.split('___')[1] for c in CAULIFLOWER_CLASS_NAMES]
    ...
    try:
        ...
        ax.set_title(f'Cauliflower Test Confusion Matrix (acc={100*test_acc:.1f}%)')
    ...
    with open(os.path.join(OUTPUT_DIR, 'confusion_matrix.json'), 'w') as f:
        json.dump({'classes': CAULIFLOWER_CLASS_NAMES, 'confusion_matrix': cm.tolist()},
                  f, indent=4)

    rows, _ = metrics_from_cm(cm)
    lines = ['cauliflower Classification Report (internal test set)', '=' * 60]
...
# ---------------------------------------------------------------------------
# Inference zero-regression spot check on existing non-cauliflower crops
# ---------------------------------------------------------------------------
...
def inference_zero_regression_check(candidate_path, n_existing=138):
    print('\n' + '=' * 60)
    print('INFERENCE ZERO-REGRESSION SPOT CHECK (existing non-cauliflower crops)')
    print('=' * 60)
    ...
    print(f'  crops the candidate now labels cauliflower (FP risk): {cauliflower_fp}/{len(paths)}')
    result = {
        'samples': len(paths),
        'max_logit_diff_first_138': max_logit_diff,
        'top1_old_class_changed': argmax_old_diff,
        'cauliflower_false_positives': cauliflower_fp,
    }
    return result
...
def main():
    ...
    print(f'  Head:          Linear(1280 -> {N_NEW}) cauliflower-only, zero-regression')
    ...
    print(f'  New classes:   indices 138-{137 + N_NEW}: {CAULIFLOWER_CLASS_NAMES}')
    print('=' * 60)
    ...
    assert [c for c in existing_classes if c.startswith('cauliflower___')] == \
        CAULIFLOWER_CLASS_NAMES, 'deployed cauliflower classes do not match!'
    candidate_classes = existing_classes[:138] + CAULIFLOWER_CLASS_NAMES
    cauli_start = 138  # deployed cauliflower rows begin at index 138
    cauli_end = cauli_start + N_NEW  # 141
    print(f'\n  Existing classes: {n_existing} | cauliflower range: {cauli_start}-{cauli_end - 1} | Candidate total: {len(candidate_classes)}')
    ...
    print(f'  Centered cauliflower biases: mean bias {bias_mean:.6f} subtracted')
    ...
    short = [c.split('___')[1][:12] for c in CAULIFLOWER_CLASS_NAMES]
    ...
    if not args.no_export:
        print('\n' + '=' * 60)
        print('CANDIDATE ONNX (cauliflower rows appended, existing rows frozen)')
        print('=' * 60)
        ...
        assert old_weight.shape[0] == cauli_end, \
            f'Expected {cauli_end} deployed rows, got {old_weight.shape[0]}'
        new_weight = np.vstack([old_weight[:cauli_start], trained_weight])
        new_bias = np.concatenate([old_bias[:cauli_start], trained_bias])
        ...
        candidate_path = os.path.join(OUTPUT_DIR, 'cauliflower_candidate.onnx')
        ...
        non_cauli = list(range(0, cauli_start))
        w_diff = float(np.max(np.abs(new_weight[non_cauli] - v1_w[non_cauli])))
        b_diff = float(np.max(np.abs(new_bias[non_cauli] - v1_b[non_cauli])))
        ok = w_diff == 0.0 and b_diff == 0.0
        print(f'  Non-cauliflower weight max diff: {w_diff:.10f}')
        print(f'  Non-cauliflower bias max diff:   {b_diff:.10f}')
        print(f'  ZERO REGRESSION: {"OK" if ok else "FAILED"}')

        inference_result = inference_zero_regression_check(candidate_path, n_existing=138)

        with open(os.path.join(OUTPUT_DIR, 'zero_regression_check.json'), 'w') as f:
            json.dump({'non_cauliflower_weight_max_diff': w_diff,
                       'non_cauliflower_bias_max_diff': b_diff,
